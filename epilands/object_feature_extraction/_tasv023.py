...
def calculate_TAS_features(object_image_data: Dict[str, np.ndarray]) -> pd.Series:
    TAS_data = [
        extract_MIELv023_tas_features(
            segCellImg=obj_img,
            mask=object_image_data["masks"],
            ch=ch
            )
        for ch, obj_img in object_image_data.items() if ch != "masks"
    ]
...
def extract_MIELv023_tas_features(
            ...
            [
                ...
                pd.Series(
                    ...
                    index=MIELv023_tas_name_features(
                        channel=ch, mask_number=0, percent=percent, n_neighbors=n_neighbors,
                    ),
                    dtype="float64",
                ),
            ]
        ...
        tas_data1 = pd.concat(
            [
                ...
                pd.Series(
                    ...
                    index=MIELv023_tas_name_features(
                        channel=ch, mask_number=1, percent=percent, n_neighbors=n_neighbors,
                    ),
                    dtype="float64",
                ),
            ]
        ...
        tas_data2 = pd.concat(
            [
                ...
                pd.Series(
                    ...
                    index=MIELv023_tas_name_features(
                        channel=ch, mask_number=2, percent=percent, n_neighbors=n_neighbors,
                    ),
                    dtype="float64",
                ),
            ]
    ...
    tas_data3 = pd.Series(
        ...
        index=MIELv023_tas_name_features(channel=ch, mask_number=3, percent=None,  n_neighbors=n_neighbors,),
        dtype="float64",
    )
