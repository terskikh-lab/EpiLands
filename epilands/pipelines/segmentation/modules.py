...
class ImageFeatureTransformer(ImageInput):
    def power_log_log_slope(self, image_data: dict):
        index = []
        data = []
        for ch, image in image_data.items():
            index.append(f"{ch}_power_LogLog_slope")
            data.append(power_spectrum_loglog_slope(image, plot_result=False))
        return pd.Series(data=data, index=index)

    def percent_max(self, image_data: dict):
        index = []
        data = []
        for ch, image in image_data.items():
            index.append(f"{ch}_percent_max")
            data.append(percent_max(image))
        return pd.Series(data=data, index=index)

    def percent_median2SD(self, image_data: dict) -> pd.Series:
        index = []
        data = []
        for ch, image in image_data.items():
            index.append(f"{ch}_percent_median2SD")
            data.append(percent_median2SD(image))
        return pd.Series(data=data, index=index)

    def run(self):
        try:

            def whole_process(image_data: Dict[str, np.ndarray]) -> List[pd.Series]:
            ...
            self.create_directory("image_features")
            output_directory = self.get_directory("image_features")
            self.segmentation_files = []
            # iterate through every file in the given file dict/segmentation channel pair to segment each set of images
            for row_col_fov, group_file_information in tqdm(
                self.image_file_information_grouped,
                ...
                logger.info(
                    f"Running image features for row{row_col_fov[0]}col{row_col_fov[1]}fov{row_col_fov[2]}"
                )
                wellidx = group_file_information.iloc[0][
                    "row", "column", "FOV", "WellIndex"
                ]
                self._load_group_image_data(
                    group_file_information=group_file_information
                )
                feature_list = whole_process(self.image_data)
                feature_list.append(wellidx)
                df_image_features = pd.concat(feature_list).T
                logger.info(f"Saving image features data to {output_directory}")
                save_dataframe_to_csv(
                ...
                self.delete_tempfile(os.path.join(output_directory, temp_file_name))
                logger.info(
                    f"Finished image features for row{row_col_fov[0]}col{row_col_fov[1]}fov{row_col_fov[2]}"
                )
        except Exception as e:
            logger.error("An exception occurred, cleaning up...")
            logger.error(e)
...
class ImageVisualization(ImageObjectTransformer):
    def randomly_select_one_image_per_well(self, **kwargs):
        selection = (
            self.image_file_information[["row", "column", "FOV"]]
            .groupby(["row", "column"])
            .agg("sample", **kwargs)
        )
        self.random_display_images = list(
            zip(selection["row"], selection["column"], selection["FOV"])
        )

    def plot_image_data(self, name, output_directory: str = "v2_plot_image_data"):
        output_directory = self.get_directory(output_directory)
        try:
            ## Show results of segmentation
            final_file_name = f"{name}_all_channels.pdf"
            temp_file_name = final_file_name.replace(".pdf", ".tmp")
            file_not_in_use = self.create_temp_file(
                final_file_name=final_file_name,
                temp_file_name=temp_file_name,
                path=output_directory,
            ...
            if file_not_in_use == False:
                logger.info(f"{name} already in progress, skipping...")
                return file_not_in_use
            # display the image data
            fig, axs = plot_image_data_dict(
                image_dict=self.image_data,
                name=f"{name}_all_channels",
                output_directory=output_directory,
                save=True,
            )
        except AttributeError as e:
            logger.error(
                "Must run self.o2_load_groupFileInformation_image_data before visualizing results"
            )
            raise e

    def plot_segmentation_mask(
        self, name, output_directory: str = "v5_plot_segmentation_mask"
    ):
        output_directory = self.get_directory(output_directory)
        try:
            file_not_in_use = self.create_temp_file(
                final_file_name=f"{name}_{self.segmentation_channel}_segmented.pdf",
                temp_file_name=f"{name}_{self.segmentation_channel}_segmented.tmp",
                path=output_directory,
            )
            if file_not_in_use == False:
                logger.info(
                    f"File {f'{name}_{self.segmentation_channel}_segmented.tmp'} already exists, skipping..."
                )
                return file_not_in_use
            # display the segmentation results
            fig, axs = plot_segmentation_mask(
                image=self.segmentation_image,
                masks=self.masks_objects_details[0],
                output_directory=output_directory,
                name=f"{name}_{self.segmentation_channel}_segmented",
                save=True,
            )
        except AttributeError as e:
            logger.error("Must run self.o5_segment_images before visualizing results")
            raise e

    def plot_illumination_correction(
        self,
        field_of_view,
        name,
        output_directory: str = "v3_plot_illumination_correction",
    ):
        output_directory = self.get_directory(output_directory)
        ## preprocess the image
        # Check if illumination correction file names exist
        try:
            for channel in self.image_data.keys():
                file_not_in_use = self.create_temp_file(
                    final_file_name=f"{name}_{channel}_illumination_correction.pdf",
                    temp_file_name=f"{name}_{channel}_illumination_correction.tmp",
                    path=output_directory,
                )
                if file_not_in_use == False:
                    logger.info(
                        f"File {f'{name}_{channel}_illumination_correction.tmp'} already exists, skipping..."
                    )
                    continue
                tmpImg = self.image_data[channel]
                # Set the image data to the corrected image data
                corImg = self.correct_illumination(
                    image=tmpImg,
                    channel=channel,
                    field_of_view=field_of_view,
                )
                illumination_example = {
                    f"{channel} Original": tmpImg,
                    f"{channel} Illumination Corrected": corImg,
                }
                fig, axs = plot_image_data_dict(
                    image_dict=illumination_example,
                    name=f"{name}_{channel}_illumination_correction",
                    output_directory=output_directory,
                    save=True,
                    cmap="inferno",
                )
    ...
    def plot_flatfield_darkfield(
        self, output_directory: str = "v1_plot_flatfield_darkfield"
    ) -> None:
        output_directory = self.get_directory(output_directory)
        for group, data in tqdm(
            self.image_file_information.groupby(["channel", "FOV"])
        ):
            group_name = join_tuple(group)
            final_file_name = f"{group_name}_flatfield_darkfield.pdf"
            temp_file_name = final_file_name.replace(".pdf", ".tmp")
            file_not_in_use = self.create_temp_file(
                final_file_name=final_file_name,
                temp_file_name=temp_file_name,
            ...
            # If only one exists, log and error and delete it.
            logger.info(f"Processing {group_name}")
            if group in self.illumination_correction_models:
                correction_model = self.illumination_correction_models[group]
            else:
                wait_until_file_exists(
                    path=self.illumination_correction_filenames[group],
                    timeout=self.timeout,
                )
                time.sleep(np.random.rand())
                # correction_model = basicpy.BaSiC.load_model(self.illumination_correction_filenames[channel, RowColFOV[-1]])
                correction_model = run_func_IO_loop(
                    func=ezload,
                    func_args={"file": self.illumination_correction_filenames[group]},
                    timeout=self.timeout,
                )
                correction_model = correction_model["model"]
            # correction_model.
            fig, axs = plot_flatfield_darkfield(
                channel_fov=group_name,
                flatfield=correction_model.flatfield,
                darkfield=correction_model.darkfield,
                output_directory=output_directory,
                filename=f"{group_name}_flatfield_darkfield",
                save=True,
            )


class ImageFeatureInput(Module):
    def __init__(
        self,
        name: str,
        segmentation_channel: str,
        ...
        output_directory: str,
        search_pattern: Union[str, re.Pattern] = ".csv",
        metadata_pattern: Union[str, re.Pattern] = re.compile(
        ...
        if segmentation_channel not in image_file_information["channel"]:
            raise ValueError(
                f"segmentation_channel is not a valid channel:\n\nGiven:{segmentation_channel}\n\nExpected one of: {image_file_information['channel'].unique()}"
            )
...
# class ImageFeatureVisualization:
#     def ds():
#         ds
