...
def bootstrap_df(
    ...
    # group_sizes = ddf_groups.size().compute()
    ...
    #     def _bootdf(df):
    #         sample = rng.choice(df.shape[0], size=num_cells, replace=with_replacement)
    #         return df.iloc[sample, :].mean(axis=0)

    # elif isinstance(metric, Callable):

    #     def _bootdf(df):
    #         sample = rng.choice(df.shape[0], size=num_cells, replace=with_replacement)
    #         return df.iloc[sample, :].std(axis=0)

    # elif isinstance(metric, Callable):

    #     def _bootdf(df):
    #         sample = rng.choice(df.shape[0], size=num_cells, replace=with_replacement)
    #         return df.iloc[sample, :].apply(metric, axis=0)
    ...
    # bootstrap_samples = dd.compute(*bootstrap_samples)
